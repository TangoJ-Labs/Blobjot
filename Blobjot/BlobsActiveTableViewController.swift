...
//            self.statusBarHeight = 0
        ...
        blobsActiveTableView.registerClass(BlobsActiveTableViewCell.self, forCellReuseIdentifier: Constants.Strings.blobsActiveTableViewCellReuseIdentifier)
        blobsActiveTableView.separatorStyle = .None
        blobsActiveTableView.backgroundColor = UIColor.clearColor()
...
//        self.tableView.contentInset = UIEdgeInsets(top: statusBarHeight, left: 0, bottom: 0, right: 0)
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier(Constants.Strings.blobsActiveTableViewCellReuseIdentifier, forIndexPath: indexPath) as! BlobsActiveTableViewCell
        ...
        if Constants.Data.locationBlobs.count >= blobIndex {
            ...
            let stringDate: String = formatter.stringFromDate(Constants.Data.locationBlobs[blobIndex].blobDatetime)
    ...
    func tableView(tableView: UITableView, editActionsForRowAtIndexPath indexPath: NSIndexPath) -> [UITableViewRowAction]? {
        ...
        let hide = UITableViewRowAction(style: .Normal, title: "Hide\nBlob") { action, index in
            ...
            Constants.Data.locationBlobs.removeAtIndex(blobIndex)
            
            // Refresh the Table View to no longer show that row
            self.blobsActiveTableView.performSelectorOnMainThread(#selector(UITableView.reloadData), withObject: nil, waitUntilDone: true)
            
            // Record the Blob hide in AWS so that the Blob no longer is downloaded for this user
            self.hideBlob(actionBlob.blobID, userID: Constants.Data.loggedInUser)
            
            // Remove the Blob from the map Blobs
            loopMapBlobsCheck: for (bIndex, blob) in Constants.Data.mapBlobs.enumerate() {
                if blob.blobID == actionBlob.blobID {
                    Constants.Data.mapBlobs.removeAtIndex(bIndex)
        ...
        hide.backgroundColor = UIColor.redColor()
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if Constants.Data.locationBlobs.count >= indexPath.row + 1 {
            self.loadBlobViewWithBlob(Constants.Data.locationBlobs[indexPath.row + 1])
        }
...
// *COMPLETE********* find username for previewBlob.blobUserID
    ...
    func hideBlob(blobID: String, userID: String) {

...
class PeopleTableViewCell: UITableViewCell {
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        cellUserName = UILabel(frame: CGRect(x: 15 + cellConnectIndicator.frame.width, y: (cellContainer.frame.height / 2) - 15, width: cellContainer.frame.width - 40 - cellConnectIndicator.frame.width - cellUserImage.frame.width, height: 30))
        cellUserName.font = UIFont(name: Constants.Strings.fontRegular, size: 16)
        cellContainer.addSubview(cellUserName)
        
        cellActionMessage = UILabel(frame: CGRect(x: 15 + cellConnectIndicator.frame.width, y: 5, width: cellContainer.frame.width - 40 - cellConnectIndicator.frame.width - cellUserImage.frame.width, height: 30))
